...
Single Zustand store with `persist` middleware writing to `AsyncStorage` (key: `granja-tycoon-save-v4`). The store holds all game state and every action. `partialize` is used to exclude action functions from serialization — never remove it or rename the storage key without bumping the version, or hydration will silently overwrite functions with `{}`, breaking all buttons.
...
2. **`partialize` in persist config** — Must exclude all action functions from the Zustand store. If omitted, functions get serialized as `{}` in localStorage, hydration overwrites them, and all button presses silently fail. Storage key is `granja-tycoon-save-v4` — bump this if the state shape changes incompatibly.
...
- Storage key: `granja-tycoon-save-v4`
