...
const BASE_GUIDE_ENTRIES: GuideEntry[] = [
  ...
  {
    id: 'system_crop_rotation',
    title: 'Crop Rotation',
    category: 'crops_fields',
    tags: ['rotation', 'soil', 'fertility', 'yield', 'planning'],
    summary: 'Crop rotation means changing what a field grows so soil pressure, pests, and nutrient drain do not stack up.',
    whyItMatters: 'A profitable farm is not just choosing the best crop today. Rotation keeps fields productive across seasons and helps avoid expensive recovery later.',
    howToUse: [
      'Prefer a different crop after harvest when the next crop still fits the season and cash plan.',
      'Use lower-drain crops or cover crops after heavy feeders to protect fertility.',
      'Treat the rotation bonus as a useful nudge, not a reason to plant an unprofitable crop.',
    ],
    mistakesToAvoid: [
      'Repeating the same high-drain crop because last season went well.',
      'Rotating into a crop that cannot be sold, stored, or watered properly.',
      'Ignoring soil repair once fertility, compaction, or organic matter starts slipping.',
    ],
    relatedEntryIds: ['system_soil_health', 'system_crop_seasons', 'problem_bad_yield'],
    visual: { kind: 'diagram', title: 'Rotation loop', nodes: ['Last crop', 'Soil need', 'Season', 'Next crop', 'Recovery'] },
    farmStateRules: [{ kind: 'soil' }, { kind: 'generic' }],
  },
  {
  ...
  {
    id: 'system_animal_genetics',
    title: 'Animal Genetics',
    category: 'animals_welfare',
    tags: ['genes', 'breeding', 'grade', 'production', 'livestock'],
    summary: 'Animal genes grade production, hardiness, growth, and value, shaping how strong future generations can become.',
    whyItMatters: 'Selective breeding turns livestock from daily income into a long-term improvement system, but weak feed and welfare still limit the payoff.',
    howToUse: [
      'Compare the four gene scores instead of looking only at the overall grade.',
      'Breed animals when you can support the offspring with feed, housing, and workers.',
      'Keep stronger genetics for production lines and sell weaker animals when cash or capacity is tight.',
    ],
    mistakesToAvoid: [
      'Breeding faster than the farm can feed or house the herd.',
      'Ignoring hardiness and growth because production looks exciting.',
      'Treating genetics as a replacement for welfare, hygiene, and the right buildings.',
    ],
    relatedEntryIds: ['system_animals_welfare', 'problem_feed_shortage', 'problem_animals_not_producing'],
    visual: { kind: 'diagram', title: 'Genetics loop', nodes: ['Parents', 'Genes', 'Offspring', 'Welfare', 'Production'] },
    farmStateRules: [{ kind: 'animal' }],
  },
  {
    id: 'animal_cow',
  ...
  },
  {
    ...
    title: 'Seed Lab',
    category: 'crops_fields',
    tags: ['seed', 'hybrid', 'generation', 'traits', 'yield'],
    summary: 'The Seed Lab combines seed batches to create improved offspring with better traits over generations.',
    whyItMatters: 'Seed breeding gives crops a progression path beyond buying more land. Better seed can raise yield, resilience, and quality without changing the whole farm plan.',
    howToUse: [
      'Hybridize seeds for crops you already know how to plant, water, harvest, and sell.',
      'Use higher-generation seed for valuable fields once the trait improvement is worth the wait.',
      'Keep ordinary seed around so experimentation does not block normal planting.',
    ],
    mistakesToAvoid: [
      'Using the lab while all fields are empty and cash is tight.',
      'Breeding a crop that is out of season with no greenhouse or future plan.',
      'Treating traits as more important than soil, water, and market timing.',
    ],
    relatedEntryIds: ['system_crop_seasons', 'system_crop_rotation', 'system_soil_health'],
    visual: { kind: 'diagram', title: 'Seed improvement', nodes: ['Parent seed', 'Hybrid job', 'Offspring', 'Trait gain', 'Planting plan'] },
    farmStateRules: [{ kind: 'generic' }],
  },
  {
  ...
  {
    id: 'system_futures_trading',
    title: 'Futures Trading',
    category: 'market_contracts',
    tags: ['futures', 'market', 'price lock', 'risk', 'delivery'],
    summary: 'Futures let you lock a crop price now for delivery later, trading price certainty for a real delivery obligation.',
    whyItMatters: 'A high price before harvest can be protected, but a failed delivery turns a clever hedge into a penalty.',
    howToUse: [
      'Use futures when the locked price is strong and the crop is already planted, stored, or very likely to arrive.',
      'Keep enough inventory separate from spot sales so the future delivery stays covered.',
      'Check deadlines against growth time, weather risk, and transport before opening a position.',
    ],
    mistakesToAvoid: [
      'Opening futures because the price is high while having no path to delivery.',
      'Selling the covered crop on the spot market before the contract settles.',
      'Using futures to gamble when cash reserves cannot absorb a penalty.',
    ],
    relatedEntryIds: ['system_market_prices', 'system_contracts', 'problem_missed_contract'],
    visual: { kind: 'diagram', title: 'Futures decision', nodes: ['High price', 'Delivery plan', 'Lock', 'Reserve stock', 'Settle'] },
  ...
  },
  {
    ...
    category: 'community_reputation',
    tags: ['csa', 'subscribers', 'community', 'boxes', 'reputation'],
    summary: 'CSA turns local trust into weekly box commitments that need steady variety and reliable fulfilment.',
    whyItMatters: 'CSA can create dependable income, but it asks the farm to plan variety, timing, and customer satisfaction instead of only chasing spot prices.',
    howToUse: [
      'Start with a small commitment until inventory variety is dependable.',
      'Watch subscriber satisfaction after each delivery.',
      'Use CSA as a reason to grow a balanced mix instead of only one high-price crop.',
    ],
    mistakesToAvoid: [
      'Promising more boxes than the farm can fill every week.',
      'Ignoring crop variety and treating CSA like a simple bulk sale.',
      'Expanding subscribers while cash, workers, or storage are already strained.',
    ],
    relatedEntryIds: ['system_market_prices', 'system_contracts', 'problem_feed_shortage'],
    visual: { kind: 'diagram', title: 'CSA loop', nodes: ['Commitment', 'Variety', 'Delivery', 'Satisfaction', 'Renewal'] },
    farmStateRules: [{ kind: 'market' }, { kind: 'inventory' }],
  },
  {
    id: 'problem_no_money',
  ...
  {
    id: 'system_legacy_path',
    title: 'Farm Legacy & Specialization',
    category: 'getting_started',
    tags: ['legacy', 'path', 'specialization', 'milestone', 'bonus'],
    summary: 'Your farm path (Crop Specialist, Livestock Rancher, Market Trader, or Balanced) unlocks production bonuses as you hit milestones.',
    whyItMatters: 'Committing to a path and meeting its milestones gives you a real mechanical advantage — up to 20% yield or production bonus at the highest tier.',
    howToUse: [
      'Choose a farm style at the start that matches your planned income mix.',
      'Check the Farm Legacy card on the dashboard to see your current tier and what is needed for the next one.',
      'Bonuses are automatic — crop yield multiplier for Crop Specialists, animal production multiplier for Livestock Ranchers.',
      'Balanced path bonuses activate when all three income streams (crops, livestock, processed) are each above threshold.',
    ],
    mistakesToAvoid: [
      'Choosing Livestock path but keeping all land in crops — path bonuses require matching your actual farm activity.',
      'Ignoring the milestones — the bonus is only active once the milestone check passes.',
      'Thinking you are locked in — you can still earn income from any source, but the bonus rewards commitment.',
    ],
    relatedEntryIds: ['first_year', 'problem_dont_know_next', 'system_animals_welfare'],
    visual: { kind: 'diagram', title: 'Path to bonus', nodes: ['Choose path', 'Build activity', 'Hit milestone', 'Bonus active'] },
    farmStateRules: [{ kind: 'generic' }],
  },
  {
    id: 'system_nitrogen_fixers',
    title: 'Nitrogen-Fixing Crops',
    category: 'crops_fields',
    tags: ['nitrogen', 'soil', 'legume', 'chickpeas', 'lentils', 'rotation'],
    summary: 'Legumes like chickpeas and lentils restore nitrogen to the soil as they grow, leaving fields in better condition than before.',
    whyItMatters: 'Most crops deplete soil fertility over time. Nitrogen fixers are the only crops that actively improve it, saving fertilizer costs and preventing soil degradation.',
    howToUse: [
      'Plant chickpeas or lentils in rotation after heavy-feeding crops like corn or wheat.',
      'After harvest, soil fertility on that parcel will be higher than before planting — check the Land Management report.',
      'Pair with composting and no-till tillage for maximum soil recovery between cash-crop seasons.',
    ],
    mistakesToAvoid: [
      'Skipping legumes entirely — without rotation, soil fertility will slowly drop and reduce yield on every crop.',
      'Planting legumes only when prices are good, then ignoring them — the soil benefit matters more than the sale price.',
      'Expecting big immediate cash returns: legumes are a long-term soil investment, not a high-margin crop.',
    ],
    relatedEntryIds: ['system_soil_health', 'system_crop_rotation', 'problem_bad_yield'],
    visual: { kind: 'diagram', title: 'Nitrogen cycle', nodes: ['Legume', 'Soil bacteria', 'Nitrogen fixed', 'Next crop benefits'] },
    farmStateRules: [{ kind: 'soil' }],
  },
  {
    id: 'system_reputation_recovery',
    title: 'Rebuilding Reputation',
    category: 'common_problems',
    tags: ['reputation', 'recovery', 'community', 'welfare', 'trust'],
    summary: 'Low reputation limits your contract access and market prices. It recovers through six factors — each has specific actions that move the needle fastest.',
    whyItMatters: 'Reputation gates export contracts, organic premiums, cooperative perks, and how easily workers and land sellers find you.',
    howToUse: [
      'Check the Reputation screen\'s Recovery Plan when your score is below 50 — it shows which factors are weakest.',
      'Fix animal welfare first: deficient rations drop welfare fastest and are the easiest quick fix.',
      'Complete active contracts on time — every on-time delivery raises Financial Reliability.',
      'Help neighbors with harvest requests to rebuild Community Standing.',
    ],
    mistakesToAvoid: [
      'Defaulting on contracts while trying to recover reputation — it undoes progress on Financial Reliability.',
      'Ignoring the Factor Breakdown — different factors respond to different actions, so fix the weakest one first.',
      'Expecting fast recovery — some factors like Historical Conduct improve slowly over many days.',
    ],
    relatedEntryIds: ['problem_cash_crunch', 'system_contracts', 'system_animals_welfare'],
    visual: { kind: 'diagram', title: 'Reputation loop', nodes: ['Weak factor', 'Targeted action', 'Score rises', 'Better access'] },
    farmStateRules: [{ kind: 'generic' }],
  },
];
