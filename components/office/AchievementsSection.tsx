...
function getProgress(
  ...
  switch (milestoneId) {
    ...
    case 'tier_b':
      return { current: state.harvestedCropIds.some(id => CROP_TYPES.find(c => c.id === id)?.tier === 'B') ? 1 : 0, target: 1 };
    case 'tier_a':
      return { current: state.harvestedCropIds.some(id => CROP_TYPES.find(c => c.id === id)?.tier === 'A') ? 1 : 0, target: 1 };
    case 'tier_s':
      return { current: state.harvestedCropIds.some(id => CROP_TYPES.find(c => c.id === id)?.tier === 'S') ? 1 : 0, target: 1 };
    default:
      return null;
  }
}
        ...
        return (
          <View key={cat.id}>
            ...
            {catMilestones.map(m => {
              ...
              const pct = progress ? Math.min(1, progress.current / progress.target) : isDone ? 1 : 0;
              ...
              return (
                <View key={m.id} style={[styles.milestoneCard, isDone && styles.milestoneCardDone]}>
                  ...
                  <View style={styles.milestoneInfo}>
                    <Text style={[styles.milestoneName, isDone && styles.milestoneNameDone]}>{m.title}</Text>
...
const styles = StyleSheet.create({
  ...
  divider:       { height: 1, backgroundColor: C.divider, marginHorizontal: S.md, marginVertical: S.sm },
